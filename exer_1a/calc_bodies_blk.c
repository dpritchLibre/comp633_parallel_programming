...
void calc_bodies_blk(struct vec_2d *r,
	...
	calc_a_tilde_blk(a_tilde, r, m, n, blk_size);
...
void calc_a_tilde_blk(struct vec_2d *a_tilde,
    ...
    for (size_t blk_low = 0; blk_low < n; blk_low += blk_size) {

	size_t blk_upp = blk_low + blk_size;

	// calculate `a_i / G` for each value of `i`
	for (size_t i = 0; (i < blk_upp - 1) && (i < n); i++) {

	    // each iteration adds the value of `f_ij` to `a_i_tilde`
	    for (size_t j = MAX(blk_low, i + 1); (j < blk_upp) && (j < n); j++) {
